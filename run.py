...
PRODUCT_INFO = {
    "现金理财": {"risk": "R1", "return": "1.5%", "term": "灵活申赎"},
    "定期存款": {"risk": "R1", "return": "2.0%", "term": "5年"},
    "短债类产品": {"risk": "R2", "return": "2.1%-2.7%", "term": "2年"},
    "固收+产品": {"risk": "R3", "return": "0.5%-8.0%", "term": "灵活申赎，建议持有2年以上"},
    "权益类产品": {"risk": "R4,R5", "return": "-4.0%-16.0%", "term": "灵活申赎，建议持有5年以上"},
    "年金险": {"risk": "R1", "return": "IRR为2.5%", "term": "退休后起领，直至身故"},
}
...
def _query_all(sql, args=()):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(sql, args)
            rows = cur.fetchall()
            _debug(f"SQL rows: {rows[:5] if rows else rows}")
            return rows
    finally:
        conn.close()
        _debug("SQL connection closed")
...
def _product_library_answer(text):
    product = _mentioned_product(text)
    if any(w in text for w in ("产品库", "有哪些产品", "产品有哪些", "可选产品", "产品列表")):
        return "、".join(PRODUCT_INFO.keys())
    if any(w in text for w in ("灵活申赎", "流动性好", "灵活赎回")):
        names = [k for k, v in PRODUCT_INFO.items() if "灵活申赎" in v["term"]]
        return "、".join(names)
    risk = re.search(r"R[1-5]", text, re.I)
    if risk and any(w in text for w in ("产品", "有哪些", "包括", "风险等级")) and not product:
        r = risk.group(0).upper()
        if any(w in text for w in ("客户", "可配置", "能买", "适合", "承受")):
            max_risk = int(r[1])
            names = [name for name, level, _ in PRODUCTS if level <= max_risk]
        else:
            names = [k for k, v in PRODUCT_INFO.items() if r in v["risk"].split(",")]
        return "、".join(names) if names else "无"
    if not product:
        return None
    info = PRODUCT_INFO[product]
    if any(w in text for w in ("风险等级", "风险评级", "风险级别", "风险")):
        return info["risk"]
    if "中枢" in text:
        rate = next((p[2] for p in PRODUCTS if p[0] == product), None)
        return _rate_text(rate) if rate is not None else info["return"]
    if any(w in text for w in ("收益率", "预期收益", "年化", "IRR", "收益")):
        return info["return"]
    if any(w in text for w in ("期限", "投资期限", "持有", "多久", "申赎", "起领")):
        return info["term"]
    if any(w in text for w in ("介绍", "特点", "信息", "是什么")):
        return f"{product}：风险等级{info['risk']}，预期收益率{info['return']}，投资期限{info['term']}"
    return None


def _is_stat_question(text):
    return any(w in text for w in (
        "平均", "最高", "最低", "最多", "最少", "占比", "比例", "数量", "多少人", "多少客户",
        "客户数", "人数", "次数", "排名", "哪个客户", "哪位客户", "购买次数", "浏览次数", "行为次数"
    ))
...
def _single_action_answer(uid, text):
    product = _mentioned_product(text)
    action_cond = ""
    if "浏览" in text:
    ...
    product_cond = f" AND {_product_filter(product)}" if product else ""
    if any(w in text for w in ("占比", "比例", "百分比")) and (product or action_cond):
        num = _query_one(
            f"SELECT COUNT(*) AS v FROM {ACTION_TABLE} WHERE user_id=%s AND prod_typ <> '非财富'{action_cond}{product_cond}",
            (uid,),
        )
        den = _query_one(
            f"SELECT COUNT(*) AS v FROM {ACTION_TABLE} WHERE user_id=%s AND prod_typ <> '非财富'",
            (uid,),
        )
        return _percent_text(num["v"] if num else 0, den["v"] if den else 0)
    if any(w in text for w in ("是否", "有没有", "有无", "买过", "购买过", "浏览过", "收藏过")) and (product or action_cond):
        row = _query_one(
            f"SELECT COUNT(*) AS v FROM {ACTION_TABLE} WHERE user_id=%s AND prod_typ <> '非财富'{action_cond}{product_cond}",
            (uid,),
        )
        cnt = int(row["v"] or 0) if row else 0
        return "是" if cnt > 0 else "否"
    if any(w in text for w in ("行为次数", "总行为", "一共多少次", "多少次", "几次", "记录数")):
        row = _query_one(
            f"SELECT COUNT(*) AS v FROM {ACTION_TABLE} WHERE user_id=%s AND prod_typ <> '非财富'{action_cond}{product_cond}",
            (uid,),
        )
        return f"{int(row['v'] or 0) if row else 0}次"
    return None
...
def _required_return(row, goal):
    months = _months_to_retire(row)
    asset = float(row.get("Net_Asset") or 0)
    surplus = float(row.get("Monthly_Income") or 0) - float(row.get("Monthly_Expend") or 0)
    if months <= 0:
        return 0.0 if asset >= goal else None
    if asset + surplus * months >= goal:
        return 0.0
    lo, hi = 0.0, 0.30
    for _ in range(80):
        mid = (lo + hi) / 2
        if _future_value(row, mid) >= goal:
            hi = mid
        else:
            lo = mid
    return hi
...
def _general_answer(text):
    if any(w in text for w in ("当前日期", "今天日期", "评估日期")):
        return "2025年3月31日"
    if "默认" in text and any(w in text for w in ("寿命", "预期寿命")):
        return "80岁"
    if "默认" in text and "通胀" in text:
        return "2%"
    if "默认" in text and any(w in text for w in ("投资回报率", "收益率", "回报率")):
        return "2%"
...
def _metric_spec(text):
    specs = [
        ...
        (("月支出", "每月支出", "支出", "消费", "花销"), "Monthly_Expend", "元"),
        (("退休金", "养老金", "第一支柱", "社保"), "Pension", "元"),
        (("企业年金", "第二支柱"), "COALESCE(Enterprise_Ann,0)", "元"),
        (("净资产", "资产"), "Net_Asset", "元"),
        (("年龄", "岁数"), "Age", "岁"),
    ]
    markers = ("平均", "均值", "人均", "最高", "最大", "最多", "最低", "最小", "最少", "合计", "总共", "总和", "一共")
    tail = ""
    for marker in markers:
        idx = text.find(marker)
        if idx >= 0:
            cand = text[idx:]
            if not tail or len(cand) < len(tail):
                tail = cand
    if tail:
...
def _base_where(text):
    ...
    fields = [
        ...
        ("第二支柱", "COALESCE(Enterprise_Ann,0)", 1),
    ]
    ops = [
        (("及以上", "以上", "不低于", "不少于", "大于等于"), ">="),
        (("超过", "大于", "高于"), ">"),
    ...
    if any(k in text for k in ("企业年金", "第二支柱")):
        if any(w in text for w in ("没有", "无", "为0", "等于0")):
            where.append("COALESCE(Enterprise_Ann,0)=0")
        elif any(w in text for w in ("有", "非0", "大于0")) and not any(w in text for w in ("有没有", "是否有")):
            where.append("COALESCE(Enterprise_Ann,0)>0")
...
def _group_rank_answer(text):
    ...
    expr, _ = metric
    order = "ASC" if any(w in text for w in ("最低", "最小", "最少")) else "DESC"
    has_rank_word = any(w in text for w in ("最高", "最低", "最大", "最小", "最多", "最少"))
    asks_risk_group = has_rank_word and not re.search(r"R[1-5]", text, re.I) and (
        any(w in text for w in ("哪个风险", "哪类风险", "哪种风险", "风险等级", "风险评级"))
        or ("风险" in text and any(w in text for w in ("哪个", "哪类", "哪种")))
    )
    if asks_risk_group:
    ...
    asks_gender_group = has_rank_word and (
        any(w in text for w in ("哪个性别", "哪种性别", "哪类性别"))
        or ("男女" in text and any(w in text for w in ("哪个", "哪类", "哪种", "谁")))
    )
    if asks_gender_group:
...
def _distribution_answer(text):
    if not any(w in text for w in ("各", "每个", "分别", "分布", "明细", "占比分布")):
        return None
    metric = _metric_spec(text)
    agg = _agg_func(text)
    wants_percent = any(w in text for w in ("占比", "比例", "百分比"))
    wants_count = any(w in text for w in ("人数", "客户数", "多少人", "多少客户", "数量", "个数"))

    if any(w in text for w in ("产品", "行为", "浏览", "购买", "收藏")):
        cond, _ = _action_filter(text)
        where = cond if cond else "prod_typ <> '非财富'"
        sql = f"""
            SELECT product AS k, COUNT(*) AS v
            FROM (
                SELECT {_product_case()}
                FROM {ACTION_TABLE}
                WHERE {where}
            ...
            WHERE product <> '其他'
            GROUP BY product
            ORDER BY v DESC, product
        """
        rows = _query_all(sql)
        if not rows:
            return None
        total = sum(int(r["v"] or 0) for r in rows)
        if wants_percent:
            return "，".join(f"{r['k']}:{_percent_text(r['v'], total)}" for r in rows)
        return "，".join(f"{r['k']}:{int(r['v'])}次" for r in rows)

    group_field = None
    if "风险" in text or "Rsk" in text or "评级" in text:
        group_field = "Rsk_Cd"
    elif "性别" in text or "男女" in text:
        group_field = "Gender"
    if not group_field:
        return None
    if metric and agg and not wants_count and not wants_percent:
        expr, unit = metric
        sql = f"SELECT {group_field} AS k, {agg}({expr}) AS v FROM {BASE_TABLE} GROUP BY {group_field} ORDER BY k"
        rows = _query_all(sql)
        return "，".join(f"{r['k']}:{_fmt_by_unit(r['v'], unit)}" for r in rows) if rows else None
    rows = _query_all(f"SELECT {group_field} AS k, COUNT(*) AS v FROM {BASE_TABLE} GROUP BY {group_field} ORDER BY k")
    if not rows:
        return None
    total = sum(int(r["v"] or 0) for r in rows)
    if wants_percent:
        return "，".join(f"{r['k']}:{_percent_text(r['v'], total)}" for r in rows)
    return "，".join(f"{r['k']}:{int(r['v'])}个" for r in rows)
...
def _tool_assist(text, uid=None, row=None):
    ...
    if general:
        return general
    lib_ans = _product_library_answer(text)
    if lib_ans and not _is_stat_question(text):
        return lib_ans
    if uid and row:
...
def run(inf):
    ...
    try:
        ...
        lib_ans = _product_library_answer(text)
        if lib_ans and not any(w in text for w in ("配置", "购买", "适合", "能买", "可配", "建议", "方案")):
            _debug("branch: product library answer")
            return lib_ans

        if any(w in text for w in ("可配置哪些产品", "能配置哪些产品", "可买哪些产品", "能买哪些产品", "适合哪些产品", "可投产品", "可用产品")):
        ...
        single_action = _single_action_answer(uid, text)
        if single_action:
            _debug(f"branch: single action answer={single_action}")
        ...
        if any(w in text for w in ("年龄", "几岁", "多大")) and "平均" not in text and "退休" not in text:
            _debug("branch: age")
            return _age(row["Age"])
        if "性别" in text:
        ...
        if "月收入" in text or "每月收入" in text or ("收入" in text and not any(w in text for w in ("退休", "养老金", "退休金", "第一支柱", "第二支柱", "企业年金"))):
            _debug("branch: monthly income")
            return _money(row.get("Monthly_Income") or 0)
        if "月支出" in text or "每月支出" in text or ("支出" in text and "退休" not in text) or ("消费" in text and "退休" not in text) or ("花销" in text and "退休" not in text):
        ...
        if "退休金" in text or ("养老金" in text and "现值" not in text) or "第一支柱" in text or "社保" in text:
        ...
        if "退休" in text and any(w in text for w in ("年龄", "几岁")) and not any(w in text for w in ("距离", "还有", "多久")):
            _debug("branch: retire age")
            ry, rm = divmod(_retire_age_months(row), 12)
            return f"{ry}岁{rm}个月"
        ...
        if "退休后" in text and any(w in text for w in ("多少个月", "几个月", "多少年", "几年")):
            _debug("branch: retirement duration")
            life = _extract_life(text, row)
            months = max(0, int(life * 12 - _retire_age_months(row)))
            y, m = divmod(months, 12)
            return f"{y}年{m}个月"
        ...
        if any(w in text for w in ("刚退休", "退休时每月", "每月需要支出", "退休后每月支出")):
            ...
            rate = _num_before(text, "%")
            if "通胀" in text and any(w in text for w in ("提升到", "提高到", "变为", "达到")):
                years = _num_before(text, "年后", 10) or 10
                rate = (_num_before(text, "%", 3) or 3) / 100
                return _money(_expense_at_retire(row, DEFAULT_INFLATION, years, rate))
            if "通胀" in text and rate is not None:
                return _money(_expense_at_retire(row, inflation=rate / 100))
        ...
        if any(w in text for w in ("最低需要积攒", "最低需要储备", "需要准备多少钱", "还差多少钱", "资金缺口", "养老缺口")):
            ...
            elif "通胀" in text and _num_before(text, "%") is not None:
                rate = _num_before(text, "%") / 100
                need, _, _, _ = _need_at_retire(row, inflation=rate, post_inflation=rate)
            else:
        ...
        if any(w in text for w in ("能否达成", "能不能达成", "是否达成", "如何调整", "怎么调整")):
            ...
            product = _mentioned_product(text)
            rate = next((p[2] for p in PRODUCTS if p[0] == product), DEFAULT_RETURN)
            if _future_value(row, rate) >= need:
                return "能达成"
            p = _lowest_goal_product(row, need)
            return f"不能，需要改为投资{p[0]}"

        if any(w in text for w in ("最低收益率", "需要收益率", "需要年化", "多少收益率", "年化多少")) and any(w in text for w in ("达成", "满足", "养老", "目标", "缺口")):
            _debug("branch: required return")
            need, _, _, _ = _need_at_retire(row)
            r = _required_return(row, need)
            return "无法达成" if r is None else _rate_text(r)
